...
public Object login(@RequestBody String jsonData) {
    ...
    try {
        ...
        String sql = "SELECT login_id loginId,coalesce(branch,'0') branch,user_name userName,status,coalesce(password,'') AS password FROM organization_user WHERE login_id='" + loginId + "' OR email='" + loginId + "'";
        DB db = new DB();
